	...
	for ( i = 1; i < length * ( length - 1 ); i++ )		// i < 90
	{
		if ( ( i % length != 0 ) || ( ( i + length - ( i % length ) ) / length ) == length )
		{
			...
			if ( connections2d[i+length][j] == 0 && connections2d[j][i+length] == 0 && !skip(i+length,j))
			{
				...
				conn2d[i+length][j] = 1;
				conn2d[j][i+length] = 1;
				chain = makebox2( conn2d, i+length, j );
				if ( chain < minchain )
				{
					minchain2=minchain;
					minchain = chain;
		...
		//excluding elements (10 , 20 ... 100) OR (90 , 91 ... 100)
		{
			...
			if ( !finddot( i , j ) )
			{
				...
				if ( !ischance() )	
				{
					if ( turn>85)
						chainlength(i,j);
					if ( even == 1 && noofchains % 2 == 0 )
					{
						connections2d[i][j] = 0;
						connections2d[j][i] = 0;
						return (findvalidindex( i , j ));
					}
					else if ( even == 0 && noofchains % 2 == 1 )
					{
						connections2d[i][j] = 0;
						connections2d[j][i] = 0;
						return (findvalidindex( i , j ));
...
// VERSION 2 for finding chainlength
	...
	if ( b+11<90)
		...
		b=90;
	for ( i = a; i < b; i++ )		// i < 90 , here length = 10
	{
		sides = 0;
		if ( i % length != 0 || (i + ( i % length ) ) / length == length )
		{
...
function move(board) {
	...
	if( best_move == 10000 )
	{
		...
		if ( best_move == 10000 )
		{
			...
			best_move = 0;
			tt = chainlength();
			//print("chain element is "+tt[0]+" "+tt[1]+" on turn "+turn);
		...
		//else
		//	print("safe element is "+valid_moves[best_move]+" on turn "+turn);
	}	
